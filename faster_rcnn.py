...
#Change for with a while and make it wait for images send via serial o socket.
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
	...
	for key in bboxes:
		...
		for jk in range(new_boxes.shape[0]):
			(x1, y1, x2, y2) = new_boxes[jk,:]#bounding boxes of image
			
			cv2.rectangle(img_scaled,(x1, y1), (x2, y2), class_to_color[key],2)

			textLabel = '{}: {}'.format(key,int(100*new_probs[jk])) #labels of class
			all_dets.append((key,100*new_probs[jk]))

			(retval,baseLine) = cv2.getTextSize(textLabel,cv2.FONT_HERSHEY_COMPLEX,1,1)
			textOrg = (x1, y1-0)

			#Add here the bounding boxes and class to send.
